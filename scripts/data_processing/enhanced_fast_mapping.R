...
source("scripts/data_processing/fast_id_mapping.R")

# Function to load all biomaRt ENSP mappings
load_biomart_ensp_mappings <- function() {
  if (file.exists(get_data_path("biomart_ensp_mappings.csv"))) {
    message("Loading biomaRt ENSP mappings...")
    biomart_data <- read.csv(get_data_path("biomart_ensp_mappings.csv"), stringsAsFactors = FALSE)
